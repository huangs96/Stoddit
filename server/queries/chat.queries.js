...
const getChatroomByUserID = "SELECT participant.account_id, chatroom.name, chatroom.description, participant.chatroom_id, account.contact_img FROM participant INNER JOIN chatroom ON participant.chatroom_id=chatroom.id INNER JOIN account ON participant.account_id = account.id WHERE account_id = $1 AND participant.deleted_at IS NULL AND chatroom.deleted_at IS NULL";
